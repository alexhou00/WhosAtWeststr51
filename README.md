...
- `config.py`: One editable place for target MAC/IP/hostname values, subnet, polling interval, bind host, and optional fallback settings.
...
```json
{
  ...
  "sources_attempted": ["ip neigh", "arp -a"],
  "details": [],
  "errors": []
}
```
...
- `ip neigh` and `arp -a` are passive. They only show devices that the Pi has seen recently.
